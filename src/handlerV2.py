...
class Handler():
    ...
    def fancy_call(self,command,only_last_line=True):
        if type(command) == str:
    ...
    def add_folder(self,folder_path="tutorial"):
        self.fancy_call("flags set -p "+folder_path+":+")

    def load_package(self,package_name="Polyfifo"):
        self.fancy_call("bpackage load "+package_name)

    def list_packages(self):
        s = self.fancy_call("bpackage list")
        packages = s.split()
    ...
    def list_types(self,package_name="Polyfifo"):
        s = self.fancy_call("defs type " + package_name )
        types = s.split()
        return types

    def list_funcs(self,package_name="Polyfifo"):
        funcs = self.fancy_call("defs func " + package_name )
        return funcs
    ...
    def read_type(self,type_string=b"Polyfifo::FIFOIfc"):
        ...
        #     return b""
        try:
            constr = self.fancy_call(b"type constr "+type_name,only_last_line=False)
        ...
        try:
            ...
            type_full = self.fancy_call(b"type full "+constr,only_last_line=False)
        except Exception as e:
            print(e)
